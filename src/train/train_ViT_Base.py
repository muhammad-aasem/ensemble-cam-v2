"""
Vision Transformer Base/16 Training Script for NIH Chest X-ray Dataset

This script implements a comprehensive ViT-Base/16 fine-tuning pipeline optimized for
...
=== MODEL ARCHITECTURE ===
- Base: timm.create_model('vit_base_patch16_224') (ImageNet pre-trained)
- Modifications:
...
- Model Checkpointing: Best weights saved to models/vit_base_best.pth

=== GRAD-CAM++ COMPATIBILITY ===
- Preserves intermediate feature maps in ViT-Base layers
...
=== MODEL PERSISTENCE ===
- Saves best model weights: models/vit_base_best.pth
...
=== OUTPUT STRUCTURE ===
models/
├── vit_base_best.pth               # Best model weights
...
class ViTBaseClassifier(nn.Module):
    """Vision Transformer Base/16 model modified for chest X-ray classification using timm."""
    ...
    def __init__(self, num_classes: int = 15, dropout_rate: float = 0.5):
        ...
        self.backbone = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=0)  # num_classes=0 to get features
...
class ModelTrainer:
    """Main training class for ViT-Base chest X-ray classification."""
    ...
    def __init__(self, config: Dict):
        ...
        # Initialize paths - models saved to models/vit_base directory
        self.data_dir = Path(config['data_dir'])
        self.models_dir = Path(config['models_dir']) / 'vit_base'
    ...
    def initialize_model(self) -> ViTBaseClassifier:
        """Initialize the ViT-Base model."""
        ...
        checkpoint_path = self.models_dir / 'vit_base_best.pth'
    ...
    def initialize_optimizer_and_scheduler(self, model: ViTBaseClassifier, 
    ...
    def train_epoch(self, model: ViTBaseClassifier, train_loader: DataLoader,
    ...
    def validate_epoch(self, model: ViTBaseClassifier, val_loader: DataLoader,
    ...
    def save_checkpoint(self, model: ViTBaseClassifier, optimizer: optim.Optimizer,
        ...
        torch.save(checkpoint, self.models_dir / 'vit_base_latest.pth')
        
        # Save best checkpoint - only save if better than previous best
        if is_best:
            torch.save(checkpoint, self.models_dir / 'vit_base_best.pth')
    ...
    def evaluate_model(self, model: ViTBaseClassifier, test_loader: DataLoader) -> Dict:
    ...
    def train(self):
        ...
        print("Starting ViT-Base training for chest X-ray classification...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train ViT-Base on NIH Chest X-ray dataset')
    ...
    print("ViT-Base Training for NIH Chest X-ray Dataset")
    print("=" * 60)
    print(f"Dataset directory: {data_dir}")
    print(f"Models directory: {args.models_dir}/vit_base")
