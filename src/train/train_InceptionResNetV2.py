"""
InceptionResNetV2 Training Script for NIH Chest X-ray Dataset

This script implements a comprehensive InceptionResNetV2 fine-tuning pipeline optimized for
...
=== MODEL ARCHITECTURE ===
- Base: torchvision.models.inception_resnet_v2 (ImageNet pre-trained)
- Modifications:
...
- Model Checkpointing: Best weights saved to models/inception_resnet_v2_best.pth

=== GRAD-CAM++ COMPATIBILITY ===
- Preserves intermediate feature maps in InceptionResNetV2 layers
...
=== MODEL PERSISTENCE ===
- Saves best model weights: models/inception_resnet_v2_best.pth
...
=== OUTPUT STRUCTURE ===
models/
├── inception_resnet_v2_best.pth    # Best model weights
...
class ModelTrainer:
    """Main training class for InceptionResNetV2 chest X-ray classification."""
    ...
    def __init__(self, config: Dict):
        ...
        # Initialize paths - models saved to models/inception_resnet_v2 directory
        self.data_dir = Path(config['data_dir'])
        self.models_dir = Path(config['models_dir']) / 'inception_resnet_v2'
    ...
    def initialize_model(self) -> InceptionResNetV2Classifier:
        """Initialize the InceptionResNetV2 model."""
        model = InceptionResNetV2Classifier(
            num_classes=len(self.class_names),
            dropout_rate=self.config['dropout_rate']
        )
        
        # Load checkpoint if available
        checkpoint_path = self.models_dir / 'inception_resnet_v2_best.pth'
    ...
    def initialize_optimizer_and_scheduler(self, model: InceptionResNetV2Classifier, 
    ...
    def train_epoch(self, model: InceptionResNetV2Classifier, train_loader: DataLoader,
    ...
    def validate_epoch(self, model: InceptionResNetV2Classifier, val_loader: DataLoader,
    ...
    def save_checkpoint(self, model: InceptionResNetV2Classifier, optimizer: optim.Optimizer,
        ...
        torch.save(checkpoint, self.models_dir / 'inception_resnet_v2_latest.pth')
        
        # Save best checkpoint - only save if better than previous best
        if is_best:
            torch.save(checkpoint, self.models_dir / 'inception_resnet_v2_best.pth')
    ...
    def evaluate_model(self, model: InceptionResNetV2Classifier, test_loader: DataLoader) -> Dict:
    ...
    def train(self):
        ...
        print("Starting InceptionResNetV2 training for chest X-ray classification...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train InceptionResNetV2 on NIH Chest X-ray dataset')
    ...
    print("InceptionResNetV2 Training for NIH Chest X-ray Dataset")
    print("=" * 60)
    print(f"Dataset directory: {data_dir}")
    print(f"Models directory: {args.models_dir}/inception_resnet_v2")
