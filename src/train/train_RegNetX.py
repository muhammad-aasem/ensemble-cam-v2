"""
RegNetX-1.6GF Training Script for NIH Chest X-ray Dataset

This script implements a comprehensive RegNetX-1.6GF fine-tuning pipeline optimized for
...
=== MODEL ARCHITECTURE ===
- Base: timm.create_model('regnetx_016') (ImageNet pre-trained)
- Modifications:
...
- Model Checkpointing: Best weights saved to models/regnetx_best.pth

=== GRAD-CAM++ COMPATIBILITY ===
- Preserves intermediate feature maps in RegNetX layers
...
=== MODEL PERSISTENCE ===
- Saves best model weights: models/regnetx_best.pth
...
=== OUTPUT STRUCTURE ===
models/
├── regnetx_best.pth                # Best model weights
...
class RegNetXClassifier(nn.Module):
    """RegNetX-1.6GF model modified for chest X-ray classification using timm."""
    
    def __init__(self, num_classes: int = 15, dropout_rate: float = 0.5):
        super(RegNetXClassifier, self).__init__()
        
        # Load pre-trained RegNetX-1.6GF from timm
        self.backbone = timm.create_model('regnetx_016', pretrained=True, num_classes=0)  # num_classes=0 to get features
...
class ModelTrainer:
    """Main training class for RegNetX chest X-ray classification."""
    ...
    def __init__(self, config: Dict):
        ...
        # Initialize paths - models saved to models/regnetx directory
        self.data_dir = Path(config['data_dir'])
        self.models_dir = Path(config['models_dir']) / 'regnetx'
    ...
    def initialize_model(self) -> RegNetXClassifier:
        """Initialize the RegNetX model."""
        model = RegNetXClassifier(
            num_classes=len(self.class_names),
            dropout_rate=self.config['dropout_rate']
        )
        
        # Load checkpoint if available
        checkpoint_path = self.models_dir / 'regnetx_best.pth'
    ...
    def initialize_optimizer_and_scheduler(self, model: RegNetXClassifier, 
    ...
    def train_epoch(self, model: RegNetXClassifier, train_loader: DataLoader,
    ...
    def validate_epoch(self, model: RegNetXClassifier, val_loader: DataLoader,
    ...
    def save_checkpoint(self, model: RegNetXClassifier, optimizer: optim.Optimizer,
        ...
        torch.save(checkpoint, self.models_dir / 'regnetx_latest.pth')
        
        # Save best checkpoint - only save if better than previous best
        if is_best:
            torch.save(checkpoint, self.models_dir / 'regnetx_best.pth')
    ...
    def evaluate_model(self, model: RegNetXClassifier, test_loader: DataLoader) -> Dict:
    ...
    def train(self):
        ...
        print("Starting RegNetX training for chest X-ray classification...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train RegNetX on NIH Chest X-ray dataset')
    ...
    print("RegNetX Training for NIH Chest X-ray Dataset")
    print("=" * 60)
    print(f"Dataset directory: {data_dir}")
    print(f"Models directory: {args.models_dir}/regnetx")
